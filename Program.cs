...
app.MapGet("/todoitems", async (TodoDbContext db) =>
    await db.TodoEntities.ToListAsync());

app.MapGet("/todoitems/complete", async (TodoDbContext db) =>
    await db.TodoEntities.Where(t => t.IsComplete).ToListAsync());

app.MapGet("/todoitems/{id}", async (int id, TodoDbContext db) =>
    await db.TodoEntities.FindAsync(id)
        is Todo todo
            ? Results.Ok(todo)
            : Results.NotFound());
...
{
    var todo = await db.TodoEntities.FindAsync(id);
...
{
    if (await db.TodoEntities.FindAsync(id) is Todo todo)
    {
        db.TodoEntities.Remove(todo);
        await db.SaveChangesAsync();
        return Results.NoContent();
    }
